...
// todo: Naughty this take these globals out
CI2CMaster *g_m_pI2CMaster;
u8 g_m_ucSlaveAddress;
struct bme280_dev dev;
uint32_t g_req_delay;
...
#endif
	m_ucSlaveAddress = 0x76;

	// todo: Remove the global
	g_m_pI2CMaster = m_pI2CMaster;
        g_m_ucSlaveAddress = m_ucSlaveAddress;
...
int8_t user_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
     ...
     * |------------+---------------------|
     */
    int nResult = g_m_pI2CMaster->Write (g_m_ucSlaveAddress, &reg_addr, 1);
    if(nResult <= 0)
      return -1;
    nResult = g_m_pI2CMaster->Read (g_m_ucSlaveAddress, reg_data, len);
    if(nResult <= 0)
      return -1;
    return rslt;
}
...
int8_t user_i2c_write(uint8_t reg_addr, const uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
    ...
    if(nResult <= 0)
      return nResult;
    nResult = g_m_pI2CMaster->Write (g_m_ucSlaveAddress, &reg_data, len);
    if(nResult <= 0)
      return nResult;
...
int8_t stream_sensor_data_forced_mode(struct bme280_dev *dev)
{
    ...
    g_req_delay = bme280_cal_meas_delay(&dev->settings);
